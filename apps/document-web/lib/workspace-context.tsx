...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (pathMatch) {
      ...
      // Clear if path doesn't have org/workspace
  ...
  useEffect(() => {
    const loadWorkspaces = async () => {
      try {
        ...
        // Fetch organization and workspace information from Console API's portal endpoint
        const consoleApiUrl = process.env.NEXT_PUBLIC_CONSOLE_API_URL || "http://localhost:10200";
        const workspacesRes = await fetch(`${consoleApiUrl}/api/portal`, {
          headers: {
        ...
        if (workspacesRes.ok) {
          ...
          // If URL doesn't have organization/workspace, select the first one and redirect
          // However, don't redirect on login or logout pages
          const pathMatch = pathname.match(/^\/org\/([^/]+)\/workspace\/([^/]+)/);
          if (
            !pathMatch &&
  ...
  const handleSetOrganizationId = (newOrgId: string | null) => {
    // If organization is changed, redirect to the first workspace of that organization
    if (newOrgId) {
  ...
  const handleSetWorkspaceId = (newWorkspaceId: string | null) => {
    ...
    // Update path-based URL (automatically re-rendered by router.push)
