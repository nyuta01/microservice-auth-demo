...
export function WorkspaceProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (pathMatch) {
      ...
      // Clear if there's no org/workspace in the path
  ...
  useEffect(() => {
    const loadWorkspaces = async () => {
      try {
        ...
        // Get organization and workspace information from Console API portal endpoint
  ...
  const handleSetOrganizationId = (newOrgId: string | null) => {
    // If organization changes, redirect to the first workspace in that organization
    if (newOrgId) {
  ...
  const handleSetWorkspaceId = (newWorkspaceId: string | null) => {
    ...
    // Update path-based URL (automatically re-renders via router.push)
